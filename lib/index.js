...
class Plugin {
	constructor(...args) {
		...
		// Inherit own enumerable properties of plugins this plugin extends
		const propNamesUsed = Object.create(null);
		if (_extends) {
			for (const plugin of _extends) {
				const pluginName = plugin.name || 'anonymous';
				for (const key of Object.keys(plugin)) {
					if (['name', 'version', 'extend', 'extends', 'dependencies'].includes(key)) continue;
					assert(
						!propNamesUsed[key],
						`Property '${key}' clash between '${propNamesUsed[key]}' and '${pluginName}' plugins`
					);
					this[key] = plugin[key];
					propNamesUsed[key] = pluginName;
				}
			}
		}
		...
		if (symbolNames) {
			...
			for (const key of Object.keys(symbols)) {
				const existing = propNamesUsed[key];
				assert(!existing, `Symbol '${key}' clashes with '${existing}' plugin`);
